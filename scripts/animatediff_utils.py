    ...
    for script in p.scripts.alwayson_scripts:
        if script.title().lower() == "controlnet":
            ...
            if p.is_api and len(cn_units) > 0 and isinstance(cn_units[0], dict):
                ...
                cn_units_dataclass = external_code.get_all_units_in_processing(
                    p)
                for cn_unit_dict, cn_unit_dataclass in zip(cn_units,
                                                           cn_units_dataclass):
                    ...
                    if cn_unit_dataclass.image is None:
                        ...
                        cn_unit_dataclass.batch_images = getattr(cn_unit_dict,
                                                                 "batch_images",
                                                                 None)
                        cn_unit_dataclass.animatediff_batch = True

                    p.script_args[
                    script.args_from:script.args_to] = cn_units_dataclass
                    cn_units = cn_units_dataclass
                    cn_units = [x for x in cn_units if x.enabled]

                p.script_args[script.args_from:script.args_to] = cn_units_dataclass

            elif len(cn_units) > 0 and hasattr(cn_units[0], "enabled"):
                cn_units = [x for x in cn_units if x.enabled]
