...
class AnimateDiffOutput:
    ...
    def output(self, p: StableDiffusionProcessing, res: Processed,
        ...
        for i in range(res.index_of_first_image, len(res.images), step):
            ...
            frame_list = [image.copy() for image in
                          res.images[i: i + params.video_length]]
            ...
            video_paths += self._save(params, frame_list, video_path_prefix,
                                      res, i)
        ...
        if len(video_paths) > 0:
            if p.is_api:
                if not AnimateDiffOutput.api_encode_pil_to_base64_hooked:
                    ...
                    logger.info(
                        "Hooking api.encode_pil_to_base64 to encode video to base64")
                ...
                res.images = self._encode_video_to_b64(video_paths) + (
                    frame_list if 'Frame' in params.format else [])
            else:
    ...
    def _add_reverse(self, params: AnimateDiffProcess, frame_list: list):
        if params.video_length <= params.batch_size and params.closed_loop in [
            'A']:
    ...
    def _interp(
        ...
        film_model_folder = os.path.join(deforum_models_path,
                                         'film_interpolation')
        ...
        film_in_between_frames_count = calculate_frames_to_add(len(frame_list),
                                                               params.interp_x)
        ...
        interp_frame_paths = sorted(
            glob.glob(os.path.join(save_folder, '*.png')))
        frame_list = []
    ...
    def _save(
        ...
        if "PNG" in params.format and (
                shared.opts.data.get("animatediff_save_to_custom",
                                     False) or getattr(params,
                                                       "force_save_to_custom",
                                                       False)):
        ...
        if "GIF" in params.format:
                    ...
                    filter_graph=(
                        {
                            ...
                            "paluse": ("paletteuse", ""),
                            "scale": ("scale",
                                      f"{frame_list[0].width}:{frame_list[0].height}")
                        },
                        [
                            ("video_in", "scale", 0, 0),
                            ("scale", "split", 0, 0),
                        ...
                        try:
                            ...
                            with exif_tool:
                                escaped_infotext = infotext.replace('\n',
                                                                    r'\n')
                                exif_tool.execute("-overwrite_original",
                                                  f"-Comment={escaped_infotext}",
                                                  video_path_gif)
            ...
            if shared.opts.data.get("animatediff_optimize_gif_gifsicle",
                                    False):
        ...
        if "WEBP" in params.format:
            if PIL.features.check('webp_anim'):
                    ...
                    exif_bytes = piexif.dump({
                        "Exif": {
                            piexif.ExifIFD.UserComment: piexif.helper.UserComment.dump(
                                infotext, encoding="unicode")
                        }})
                lossless = shared.opts.data.get("animatediff_webp_lossless",
                                                False)
                quality = shared.opts.data.get("animatediff_webp_quality", 80)
                logger.info(
                    f"Saving {video_path_webp} with lossless={lossless} and quality={quality}")
                imageio.imwrite(video_path_webp, video_array, plugin='pillow',
                                duration=int(1 / params.fps * 1000),
                                loop=params.loop_number,
                ...
                logger.warn(
                    "WebP animation in Pillow requires system WebP library v0.5.0 or later")
